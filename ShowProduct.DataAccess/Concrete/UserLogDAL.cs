...
{
    public class UserLogDAL : EntityRepositoryBase<UserLog, ApplicationDbContext>, IUserLogDAL
    {
        public void SetActive(int id)
        {
            using (ApplicationDbContext context = new ApplicationDbContext())
            {
                var active = context.Set<UserLog>().Where(i => i.Id == id).FirstOrDefault();
        ...
        public void SetDeleted(int id)
        {
            using (ApplicationDbContext context = new ApplicationDbContext())
            {
                var deleted = context.Set<UserLog>().Where(i => i.Id == id).FirstOrDefault();
