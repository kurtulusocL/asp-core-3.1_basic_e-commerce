...
{
    public class LocationDAL : EntityRepositoryBase<Location, ApplicationDbContext>, ILocationDAL
    {
        public void SetActive(int id)
        {
            using (ApplicationDbContext context = new ApplicationDbContext())
            {
                var active = context.Set<Location>().Where(i => i.Id == id).FirstOrDefault();
        ...
        public void SetDeleted(int id)
        {
            using (ApplicationDbContext context = new ApplicationDbContext())
            {
                var deleted = context.Set<Location>().Where(i => i.Id == id).FirstOrDefault();
