...
{
    public class VideoAdDAL : EntityRepositoryBase<VideoAd, ApplicationDbContext>, IVideoAdDAL
    {
        public VideoAd HitRead(int? id)
        {
            using (ApplicationDbContext context = new ApplicationDbContext())
            {
                var hit = context.Set<VideoAd>().Where(i => i.Id == id).SingleOrDefault();
                hit.Hit++;
                context.SaveChanges();
                return hit;
            }
        ...
        public void SetActive(int id)
        {
            using (ApplicationDbContext context = new ApplicationDbContext())
            {
                var active = context.Set<VideoAd>().Where(i => i.Id == id).FirstOrDefault();
        ...
        public void SetDeleted(int id)
        {
            using (ApplicationDbContext context = new ApplicationDbContext())
            {
                var deleted = context.Set<VideoAd>().Where(i => i.Id == id).FirstOrDefault();
